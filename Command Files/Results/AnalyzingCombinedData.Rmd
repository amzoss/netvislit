...
stats_datasets_tall %>% filter(filename=="PilotStudents") %>% filter(DatasetOrder < 3) %>% group_by(`Demo-ResponseID`) %>% summarise(totalDuration=sum(DatasetDuration)/60) %>% ggplot() + geom_boxplot(aes(y=totalDuration,x=1))

#TODO: calculate actual median time expected for 2 datasets - should be around 11.5 or 12!
