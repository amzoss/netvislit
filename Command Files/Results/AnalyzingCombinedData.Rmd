...
# TO DO: Should probably do this in the processing script

stats_demo <- stats_demo %>% mutate_all(funs(ifelse(.=="-99",NA,.)))
...
# TO DO: Factor node_lookcup$Condition
...
num_ans_lim <- graded_num_ans %>% filter(QType == "Exp") %>% select(`Demo-ResponseID`, Condition, Dataset, Task, Response, DatasetOrder, DatasetDuration, DatasetStartTime, TaskOrder, CorrectAnswer, RawDifference, AbsDifference, Percentage, LogError, ClustConf, filename, `Stats-Q_TotalDuration`, `Stats-CompensationCondition`, `Demo-age`, `Demo-educ`, NetVisExperience)

nodes_lim <- graded_nodes %>% filter(QType == "Exp") %>% select(`Demo-ResponseID`, Condition, Dataset, Task, Attempt, NodeValue, DatasetOrder, DatasetDuration, DatasetStartTime, TaskOrder, MaxValue, RawDifference, AbsDifference, Percentage, LogError, filename, `Stats-Q_TotalDuration`, `Stats-CompensationCondition`, `Demo-age`, `Demo-educ`, NetVisExperience)
