...
# Do for each categorical predictor? Final predictors:
# Condition, Dataset, Condition:Dataset, Dataset:Underestimated, DatasetOrder:Underestimated

# old code: lmerTest is deprecated

# LSMeans Difference data

graph.numhd.difflsm.cond <- difflsmeans(graph.numhd.lmer.full.int.4, test.effs = "Condition")

plot(graph.numhd.difflsm.cond)

graph.numhd.difflsm.cond.df <- as_data_frame(graph.numhd.difflsm.cond$diffs.lsmeans.table,
                                             rownames="Condition.Difference")

graph.numhd.difflsm.cond.df

graph.numhd.difflsm.cond.df$Condition.Difference <- 
  factor(graph.numhd.difflsm.cond.df$Condition.Difference, 
         levels=graph.numhd.difflsm.cond.df %>% arrange(desc(Estimate)) %>% 
           dplyr::select(Condition.Difference) %>% unlist())

graph.numhd.difflsm.cond.df %>% arrange(desc(Estimate))

ggplot(graph.numhd.difflsm.cond.df) +
  geom_point(aes(x=Condition.Difference,y=Estimate, color=`p-value`<.01)) +
  geom_errorbar(aes(x=Condition.Difference,ymax=`Upper CI`,ymin=`Lower CI`), width=.2) +
  coord_flip()

# LSMeans data; note = lmerTest::lsmeans will only report lsmeans for factor variables
# and is deprecated

graph.numhd.lsmlt.cond <- lmerTest::lsmeans(graph.numhd.lmer.full.int.4, "Condition")

plot(graph.numhd.lsmlt.cond)

# LSMeansLT data

graph.numhd.lsmlt.cond.2 <- lsmeansLT(graph.numhd.lmer.full.int.4, test.effs = "Condition")

plot(graph.numhd.lsmlt.cond.2)
...
ggplot(graph.numhd.lsmlt.cond.2.df) +
  geom_point(aes(x=Condition,y=Estimate, color=`p-value`<.01)) +
...
graph.numhd.difflsmlt.cond.df$Pair <- factor(graph.numhd.difflsmlt.cond.df$Pair, levels=graph.numhd.difflsmlt.cond.df %>% arrange(desc(Estimate)) %>% dplyr::select(Pair) %>% unlist())
...
```{r}
# Do for each categorical predictor? Final predictors:
# Condition, Dataset, Condition:Dataset, Dataset:Overestimated, DatasetOrder:Overestimated
...
graph.numhd.lsm.condition <- emmeans(graph.numhd.lmer.full.int.4, "Condition", nesting = NULL)

graph.numhd.lsm.condition

graph.numhd.lsm.condition.df <- as.data.frame(graph.numhd.lsm.condition)

graph.numhd.lsm.condition.df

graph.numhd.lsm.dataset <- emmeans(graph.numhd.lmer.full.int.4, "Dataset", nesting = NULL)
#graph.numhd.lsm.dataset <- emmeans(graph.numhd.lmer.full.int.4, "Dataset", lmer.df = "satterthwaite")

graph.numhd.lsm.dataset

emmip(graph.numhd.lmer.full.int.4, ~Dataset, CIs = TRUE)
emmip(graph.numhd.lsm.dataset, ~Dataset, CIs = TRUE)
plot(graph.numhd.lsm.dataset)

#plot(ref_grid(graph.numhd.lmer.full.int.4), by="Condition") 
# try to figure this out? maybe only works on the interaction?

graph.numhd.difflsm.dataset <- emmeans(graph.numhd.lmer.full.int.4, pairwise ~ Dataset)

graph.numhd.difflsm.dataset

graph.numhd.difflsm.conddata <- emmeans(graph.numhd.lmer.full.int.4, pairwise ~ Dataset | Condition)

graph.numhd.difflsm.conddata
