...
TO DO : figures for non-lmer models
...
ggplot(graph.avgdeg.lmer.full.f, aes(.fitted,LogError)) + 
  geom_point() +
...
fit %<>% recover_types(graphics_lgclust.2)

print(fit, pars = c("b_Intercept"))
...
lmerTest::lsmeansLT(fit,"Dataset")

hyp <- c("exp(Dataset.L) = 0",
         "exp(Dataset.Q) = 0")
hypothesis(fit, hyp)

hyp <- "exp(Dataset.L + Dataset.Q) > exp(Dataset.L)"
(hyp <- hypothesis(fit, hyp))
...
ggplot(df_fit, aes(Dataset, M)) + geom_pointrange(aes(ymin=M-SE, ymax=M+SE)) + 
...
TO DO : confirm pairs calculation; not convinced using intercept for dataset 1 is right; maybe need to refit model so Dataset doesn't get split???
...
# Here, b_Intercept is the global mean, and the r_condition parameters are offsets from that mean for each condition

fit %>%
  gather_samples(b_Dataset.L, b_Dataset.Q, b_Dataset.C, b_DatasetE4, b_DatasetE5) %>%
  head(10)
...
graph.numhd.emm.cond <- emmeans(graph.numhd.lmer.full.int.4, "Condition", nesting = NULL)
#graph.numhd.emm.condition <- emmeans(graph.numhd.lmer.full.int.4, "Condition", lmer.df = "satterthwaite")
...
graph.numhd.emm.data <- emmeans(graph.numhd.lmer.full.int.4, "Dataset", nesting = NULL)
#graph.numhd.emm.condition <- emmeans(graph.numhd.lmer.full.int.4, "Condition", lmer.df = "satterthwaite")
...
#graph.numnodes.emm.cond <- emmeans(graph.numnodes.lmer.int.3, "Condition", nesting = NULL)
graph.numhd.emm.conddata <- emmeans(graph.numhd.lmer.full.int.4, ~ Dataset | Condition, nesting = NULL)
...
#graph.numnodes.emm.cond <- emmeans(graph.numnodes.lmer.int.3, "Condition", nesting = NULL)
graph.numhd.emm.dataover <- emmeans(graph.numhd.lmer.full.int.4, ~ Dataset | Overestimated, nesting = NULL)
...
NOTE: processed as continuous

TO DO : try model (all models???) with DatasetOrder as ordered factor
...
#graph.numnodes.emm.cond <- emmeans(graph.numnodes.lmer.int.3, "Condition", nesting = NULL)
graph.numhd.emm.dorderover <- emmeans(graph.numhd.lmer.full.int.4, ~ DatasetOrder | Overestimated, nesting = NULL)
...
graph.numnodes.emm.condunder.cld.df$Condition <- factor(graph.numnodes.emm.condunder.cld.df$Condition, levels=rev(cond.lev))
