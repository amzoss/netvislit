...
# for some reason, the last column in each of the parsed csv files was completely null, so need to remove before setting names

for (file in names(all_responses)){
  nm <- unlist(new_names[[file]])
  all_responses[[file]] <- all_responses[[file]] %>% select(-ncol(all_responses[[file]]))
...
stats_datasets_tall <- left_join(stats_datasets_tall, dataset_order, by=c("Demo-ResponseID","Dataset"))
...
graded_num_ans <- dplyr::mutate(joined_num_ans, 
                                Difference = abs(CorrectAnswer - Response), 
...
graded_num_ans[graded_num_ans$Task != "NumClust",12] <- NA
